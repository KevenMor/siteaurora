    ...
    <section className="bg-aurora-bg px-4 md:px-6 py-16 md:py-24">
      <div className="max-w-[88rem] mx-auto">
        <div className="mb-10 md:mb-16">
          <p className="text-aurora-navy/60 text-xs md:text-sm mb-2">Governança</p>
          <h2
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {items.map((item) => (
            <div key={item.title} className="rounded-2xl bg-aurora-surface p-5 md:p-7 min-h-44 md:min-h-56 flex flex-col justify-between">
              <item.icon className="w-6 h-6 md:w-7 md:h-7 text-aurora-navy/70 mb-4 md:mb-6" />
              <div>
                <h3 className="text-aurora-navy text-base md:text-lg font-medium mb-1.5 md:mb-2" style={{ letterSpacing: '-0.02em' }}>
                  {item.title}
                </h3>
                <p className="text-aurora-navy/60 text-sm md:text-base leading-relaxed">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
